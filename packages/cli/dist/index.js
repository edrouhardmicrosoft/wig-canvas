...
import { getTransportConfig, getSocketPath, getPidFilePath, isSuccessResponse, } from '@wig/canvas-core';
    ...
    if (running) {
        try {
            const response = await withClient(async (client) => {
                return client.send('daemon.status', {});
            });
...
daemonCmd
    ...
    try {
        const response = await withClient(async (client) => {
            return client.send('daemon.stop', {});
        });
        if (isSuccessResponse(response)) {
...
program
    ...
    try {
        const response = await withClient(async (client) => {
            return client.send('connect', { url });
        });
        if (isSuccessResponse(response)) {
            if (options.format === 'json') {
                console.log(JSON.stringify(response.result, null, 2));
                ...
                if (response.result.viewport) {
                    console.log(`  Viewport: ${String(response.result.viewport.width)}x${String(response.result.viewport.height)}`);
                }
            }
        }
        else {
    ...
    try {
        ...
        if (isSuccessResponse(response)) {
            if (options.format === 'json') {
                ...
                if (session.viewport) {
                    console.log(`  Viewport:  ${String(session.viewport.width)}x${String(session.viewport.height)}`);
                }
            }
        }
        else {
